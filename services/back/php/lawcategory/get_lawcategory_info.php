<?php

    include "../common/dbconfig.php";
    include "../common/utilities.php";

    $errors = [];
    $data = array();

    $cid = $_GET["cid"];

    $query = "select * from tbl_lawcategory ";
    $query .= "where lawcategory_id=$cid ";
    $query .= "order by lawcategory_id desc";

    try {
        $conn = getConnection();
        $stmt = $conn->prepare($query);
        $stmt->setFetchMode(PDO::FETCH_ASSOC); // set the resulting array to associative

        $stmt->execute();
        $rowctr = $stmt->fetchAll();  
        if (count($rowctr) < 1) {
            echo getResponse(false, "There are no Law category #$cid!", "Law category #$cid Not Found.");
            $conn = null;
            die();
        }

        $stmt->execute();
        $results = $stmt->fetch();
        echo getResponse(json_encode($results), "Successfully retrieved Law category #$cid!", "Law category #$cid Found.");

    } catch(PDOException $e) {
        echo getResponse(false, "Server Error! dblawcat", "Database Exception - " . $e->getMessage());

    }
    $conn = null;

?>